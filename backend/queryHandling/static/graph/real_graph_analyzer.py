...
class RealGraphLearningAnalyzer:
    def __init__(self, graph_file=None):
        ...
        try:
            ...
            # Build lookup dictionaries early to be available even if clustering fails
            ...
            # Perform clustering in a try-except block to handle potential errors
            try:
                self.clusters = self.perform_intelligent_clustering()
            except Exception as e:
                print(f"⚠️ Error during clustering initialization (non-critical): {e}")
                # Create a default clustering with all topics in one cluster
                self.clusters = {0: [topic['id'] for topic in self.topics]}
                
        except Exception as e:
            print(f"❌ Error initializing RealGraphLearningAnalyzer: {e}")
            import traceback
            traceback.print_exc()
            
            # Set default values so the analyzer can still function in a degraded mode
            self.graph_file = graph_file
    ...
    def perform_intelligent_clustering(self):
        ...
        # Use simple approach for fast startup - group by basic categories
        try:
            # Simple clustering based on topic categories or basic relationships
            clusters = {}
            cluster_id = 0
            ...
            # Group topics by their first character (simple approach)
            for topic in self.topics:
                topic_id = topic['id']
                topic_name = topic.get('name', '').lower()
                
                # Create simple clusters based on first letter
                if topic_name:
                    first_letter = topic_name[0]
                    if first_letter not in clusters:
                        clusters[first_letter] = []
                    clusters[first_letter].append(topic_id)
                else:
                    # Fallback for topics without names
                    if 'default' not in clusters:
                        clusters['default'] = []
                    clusters['default'].append(topic_id)
            
            print(f"✅ Created {len(clusters)} simple topic clusters for faster startup")
            return clusters
            
        except Exception as e:
            print(f"⚠️ Error during topic clustering (non-critical): {e}")
            # No need for full traceback to keep logs cleaner
            
            # Return a simple fallback clustering (all topics in one cluster)
            default_clusters = {0: [topic['id'] for topic in self.topics]}
            print(f"✅ Using fallback clustering with 1 cluster")
